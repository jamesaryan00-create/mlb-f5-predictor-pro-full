...
async function getTeamRankingsF5({ force = false, url = process.env.TEAMRANKINGS_F5_URL || DEFAULT_URL } = {}) {
  if (!force && cache.url === url && cache.rows && Date.now() - cache.at < CACHE_MS) {
  ...
  try {
    ...
    cache = { at: Date.now(), rows, error: null, url };
    ...
    return { available: false, rows: null, cached: false, reason: cache.error, source: url };
  }
}
