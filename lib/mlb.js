...
async function getPredictions(date = todayPacific()) {
  ...
  const predictions = await Promise.all(games.map(async (game) => {
    ...
    const isFinal = /final|game over/i.test(String(game.status || ''));
    const f5OddsPayload = isFinal ? { available: false, game: null, reason: 'Game is final' } : await getF5OddsForEvent(oddsGame);
