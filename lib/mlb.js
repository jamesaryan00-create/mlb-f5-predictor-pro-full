...
function calculateGamePrediction(game, inputs, oddsGame) {
  ...
  const mlHomeBase = mlProbability({ homeWinPctDiff: (homeOffense - awayOffense) / 50, homeF5RunDiff: edge / 25, homeRecentRunDiff: (homeHandSplit - awayHandSplit) / 50, homePitchingRunDiff: (homePitcher - awayPitcher) / 50, homeParkFactor: parkFactor - 1, homeRestDiff: (awayBullpen - homeBullpen) / 50 });
  ...
  const f5Overlay = mlHomeBase === null
    ...
    : teamRankingsF5.rows
      ? adjustMlProbabilityWithTeamRankings({
          ...
          homeF5RunWeight: loadModel()?.weights?.homeF5RunDiff ?? 0.671054
        })
  ...
  return {
    ...
    prediction: { pick: pickTeam.name, opponent: opponent.name, edge, confidence: confidenceFromEdge(edge), label: labelFromEdge(edge), modelProbability: Number((blendedProb * 100).toFixed(1)), ruleProbability: Number((modelProbRule * 100).toFixed(1)), mlProbabilityBase: mlPickProbBase === null ? null : Number((mlPickProbBase * 100).toFixed(1)), mlProbability: mlPickProb === null ? null : Number((mlPickProb * 100).toFixed(1)), f5Context: f5Overlay.available ? { available: true, source: teamRankingsF5.source, fetchedAt: teamRankingsF5.fetchedAt, cached: Boolean(teamRankingsF5.cached), probabilityDeltaPct: mlPickProbBase === null ? 0 : Number(((mlPickProb - mlPickProbBase) * 100).toFixed(2)), homeProbabilityDeltaPct: Number((f5Overlay.probabilityDelta * 100).toFixed(2)), contextualRunDiff: Number(f5Overlay.contextualRunDiff.toFixed(3)), home: f5Overlay.home, away: f5Overlay.away } : { available: false, reason: f5Overlay.reason || teamRankingsF5.reason }, bestMoneyline: pickLine?.price ?? null, bestBook: pickLine?.book ?? null, estimatedEV: ev, note: pickLine ? 'EV compares blended model probability against the best available full-game moneyline. Configure F5 markets if your odds provider supports them.' : 'Odds unavailable. Add ODDS_API_KEY to show moneylines and estimated value.' },
  };
}
...
async function getPredictions(date = todayPacific()) {
  ...
  const [odds, teamRankingsF5] = await Promise.all([
  ...
  const predictions = await Promise.all(games.map(async (game) => {
    ...
    return calculateGamePrediction(game, { homeTeam, awayTeam, homePitcher, awayPitcher, homeBio, awayBio, homeSplits, awaySplits, bullpen, weather, injuries, umpire, teamRankingsF5 }, findOddsGame(odds, game));
  }));
  return { date, generatedAt: new Date().toISOString(), model: loadModel(), odds: { available: odds.available, reason: odds.reason || null, markets: odds.markets || null }, dataSources: { injuries, umpire, teamRankingsF5: { available: Boolean(teamRankingsF5.available), source: teamRankingsF5.source || null, fetchedAt: teamRankingsF5.fetchedAt || null, cached: Boolean(teamRankingsF5.cached), reason: teamRankingsF5.reason || null } }, games: predictions.sort((a, b) => Math.abs(b.prediction.edge) - Math.abs(a.prediction.edge)) };
}
