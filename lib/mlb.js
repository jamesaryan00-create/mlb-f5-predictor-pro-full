...
function getRecordedPickAndResult(gamePk, date) {
  ...
  // See FEATURE-WISHLIST.md #31: picks files written before the tracker was fixed have no
  // trackedModel field and recorded the F5 model, not the site's full-game primary pick -- label
  // that plainly rather than presenting an old F5 pick/result as if it were the full-game one
  // (an F5 tie is normal; a "full-game" tie would look like a bug).
  const trackedModel = picks?.trackedModel === 'full-game' ? 'full-game' : 'f5-legacy';
  const recordedPick = { pick: pick.pickTeam ?? null, side: pick.pickSide ?? null,
    ...
    capturedAt: pick.capturedAt || pick.filledAt || null, modelVersion: pick.model?.version ?? null, trackedModel };
  ...
  const recordedResult = grade && grade.status === 'graded' ? { result: grade.result, win: grade.win, tie: grade.tie, trackedModel } : null;
